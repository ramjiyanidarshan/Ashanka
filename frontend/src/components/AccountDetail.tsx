...
interface AccountDetailProps {
  ...
  onMoveToVault?: (account: Account, isVault: boolean) => void;
}
          ...
          <div key={account._id} className="account-card" id={`account-card-${account._id}`}>
            <div className="account-card-header">
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexShrink: 0, flexWrap: "wrap", justifyContent: "flex-end" }}>
                ...
                {onMoveToVault && (
                  <button
                    id={`vault-account-${account._id}`}
                    className="btn btn-secondary btn-sm"
                    onClick={() => onMoveToVault(account, !account.isVault)}
                    title={account.isVault ? "Move out of सन्दूक" : "Move to सन्दूक"}
                  >
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
