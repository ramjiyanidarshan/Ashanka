...
import { settingsApi, sessionsApi, authApi, Session } from "@/lib/api";
import { useTheme } from "@/context/ThemeContext";

const BACKEND = process.env.NEXT_PUBLIC_BACKEND_URL ?? "http://localhost:3001";

interface Settings {
  security: { username: string; mfaEnabled?: boolean; role?: string };
...
export default function SettingsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!userRes.authenticated) { router.push("/login"); return; }
      setUserRole(userRes.role || "enduser");
  ...
  let navGroups: { label: string; keys: SectionKey[] }[] = [
  ...
  if (userRole !== "admin") {
    navGroups = [
      { label: "Account", keys: ["security"] },
      { label: "System", keys: ["sessions"] },
      { label: "Preferences", keys: ["appearance", "data"] },
    ];
  }
              ...
              <>
                ...
                {activeSection === "security" && (
                  <div className="settings-section settings-section-animate">
                    <SectionTitle icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>} desc="Manage authentication, 2FA, and password policies for your account">
                      Security Settings
                    </SectionTitle>
                    ...
                    <div className="settings-panel-card">
                      {/* Account */}
                      <Card>
                        <CardHeader icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>} title="Your Account" subtitle="Basic profile credentials" />
